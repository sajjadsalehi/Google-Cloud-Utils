...
def trigger_dag(data, context=None):
    ...
    dag_name = data
    # Airflow REST API Reference = https://airflow.apache.org/docs/stable/rest-api-ref.html
    webserver_url = ('https://{}.appspot.com/api/experimental/dags/{}/dag_runs'.format(webserver_id,dag_name))

    # Make a POST request to IAP which then Triggers the DAG
    make_iap_request(webserver_url, client_id, method='POST', json={"conf": "", "run_id": "K_{}".format(int(time.time()))})
...
def make_iap_request(url, client_id, method='GET', **kwargs):
    ...
    google_open_id_connect_token = get_token(Request(), client_id)
...
def get_token(request, client_id):
    if not hasattr(get_token, 'token'):
        get_token.token = id_token.fetch_id_token(request, client_id)
...
def main():
    ...
    for x in range(1, 11):
        start = time.time()
        trigger_dag("process{}".format(str(x).zfill(2)))
        stop = time.time()
        print("[{}] triggering DAG - elapsed {}ms".format(x, int((stop-start)*1000)))

        #start = time.time()
        #time.sleep(0.5)
        #stop = time.time()
        #print("[{}] sleeping time  - elapsed {}ms".format(x, int((stop-start)*1000)))
